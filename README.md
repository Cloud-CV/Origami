...
`origami.env` stores all of the environment variables necessary to run Origami:

1. `HOST` should be set to the hostname of the server
2. `PORT` should be set to the port you want the server to listen on (usually 80).
3. `DB_NAME` will be used to set the name of your Postgres database.
4. `DB_PASS` will be used to set the password for the database user. This is also the admin password.
5. `DB_USER` is the username for a user which can modify the database. This is also the admin username.
6. `DB_USER_EMAIL` stores the admin's email.
7. `DB_HOST` should be set to `postgres` in production and `localhost` in development.
8. `REDIS_HOST` should be set to `redis` in production and `localhost` in development.

To create the file, run `cp origami.env.sample origami.env` and enter the above fields.

### Additional configuration

`Origami/outCalls/config.js` stores the configuration variables needed by the UI:
...
3. `DROPBOX_API_KEY` - check out step 3 of [configuring Origami](http://cloudcv-origami.readthedocs.io/en/latest/web-app.html#configuration). 
...
1. Make sure your run tests on your changes before you commit push them by running:
...
2. Fix lint issues with the code using: `yarn run lint:fix`.
