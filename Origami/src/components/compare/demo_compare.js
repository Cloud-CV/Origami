...
class DemoCompare extends React.Component {
  ...
  componentWillMount() {
    ...
    for(var i =0;i<demo.length;i++){
      ...
      output.push( 
        <div style={{paddingRight:'5px',backgroundColor:'Red'}}>
        <TextOutput
        headers={"abc"}
    ...
    let fin_output=[];
        while (output.length) {
      fin_output.push(output.splice(0, 2));
    }
  ...
  render() {
    ...
    let samp=["yes","no","maybe"]
    let user=["vpn1997","vpn1996","fristonio"]
                        ...
                        { output.map((row, index) => (
                            <div>
                              <div className="two column row"  >
                                {row.map((value, ind) => (
                                  <div className="column" style={{marginRight:"17%"}}>
                                          <TextOutput
                                          calling_context={'demo'}
                                          data={samp[ind]}
                                          creator={user[ind]}
