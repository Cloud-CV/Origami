...
class DemoViewSetTests(TestCase):
	...
	def test_get_queryset(self):
		...
		self.assertEqual(list(response),list(self.demo_object))
class InputComponentViewSetTests(TestCase):
	...
	def setUp(self):
		...
		self.input_object = InputComponents.objects.create(base_component_id=self.inpcomp["base_component_id"],
									   props= self.inpcomp["props"], 
									   demo = self.inpcomp["demo"],
									    user_id = self.inpcomp["user_id"])
		
		serializer_class = InputComponentSerializer
		 
	def test_get_queryset(self):
		
        payload = self.inpcomp
        ...
        self.assertEqual(list(response), list(self.input_object))
	
		
	
	def test_user_input_component(self): #what params to pass
		test_input = self.input_object
		test_serialize = InputComponentSerializer(test_input)
		self.assertEqual(InputComponentViewSet(ModelViewSet), Response(test_serialize.data, status = test_response))
class OutputComponentViewSetTest(TestCase):
	def setUp(self):
		self.client = Client()
        ...
        self.demo_object = Demo.objects.create(name=self.demo["name"], id=self.demo["id"],
                                               ...
                                               terminal=self.demo[
                                                   "terminal"], timestamp=self.demo["timestamp"],
                                               token=self.demo["token"], status=self.demo["status"])
		self.outcomp = {base_component_id : 99,
					    props : "props",
						demo: self.demo_object,
						user_id:99
		...
		self.output_object = OutputComponent.objects.create(base_component_id=self.inpcomp["base_component_id"],
									   props= self.outcomp["props"], 
									   demo = self.outcomp["demo"],
									   user_id = self.outcomp["user_id"])
		
		serializer_class = OutputComponentSerializer
	def test_get_queryset(self):
		payload = self.outcomp
        url = '/api/outputcomponent/' + str(payload["user_id"])
        responses = self.client.get(url)
      
        response = json.loads(responses.content.decode('utf-8'))[0]
        self.assertEqual(list(response), list(self.output_object))
	
	def test_user_output_component(self):
		test_output = self.output_object
		test_serialize = OutputComponentSerializer(test_output)
		self.assertEqual(OutputComponentViewSet(ModelViewSet), Response(test_serialize.data, status = test_response))

class PermalinkViewSetTests(TestCase):
	def setUp(self):
		self.client = Client()
		self.permalink = {
        ...
        self.perma_object =Permalink.objects.create(short_relative_url=self.permalink["short_relative_url"],
	...
	def test_get_queryset(self):
		...
		self.assertEqual(list(response),list(self.perma_object))
	

class RootSettingsViewSet(TestCase):
	def setUp(self):
		self.client = Client()
		self.root = {
			"root_user_github_login_id": 12,
			"root_user_github_login_name": "jvent",
			"client_id": "hello",
		...
		self.root_object =RootSettings.object.create(root_user_github_login_id = self.root["root_user_github_login_id"],
								    root_user_github_login_name = self.root["root_user_github_login_name"],
									client_id = self.root["client_id"],
									client_secret = self.root["client_secret"],
									is_cloudcv = self.root["is_cloudcv"],
									allow_new_logins= self.root["allow_new_logins"],
									app_ip = self.root["app_ip"],
									port = self.root["port"]
	...
	def test_get_queryset(self):
        ...
        response = self.client.get('/api/rootsettings/user/99')
        responses = json.loads(response.content.decode('utf-8'))
        response = responses[0]
		self.assertEqual(list(response), list(self.root_object))
